...
/*   Updated: 2025/09/04 18:31:36 by shimi-be         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int								init_execute(t_token *node, t_token *head,
									t_env **env, int *exit_status);
int								pre_exec(char *line, t_env **env,
									int *exit_status);
...
int								is_sal(char *str);
int								init_quote_vars(char **arr, int *quote,
									int type, int *i);
...
void							assign_after_var(char *str, t_quote_vars *vars,
									int *i);
...
void							add_slash(t_struct_var *structs,
									t_quote_vars *vars);
void							add_single(t_struct_var *structs,
									t_quote_vars *vars);
void							signals_child(void);
void							handle_nested_status(t_struct_var *structs,
									t_quote_vars *vars, char *str, int *i);
void							add_status(t_struct_var *structs,
									t_quote_vars *vars);
void							handle_sigint_heredoc(int sig_int);
void							allocate_word_arr(char **arr, char *str, int *i,
									int j);
void							allocate_after_single(char *str,
									t_quote_vars *vars, int *i);
void							handle_nested_var(t_struct_var *structs,
									t_quote_vars *vars, char *str, int *i);
void							allocate_in_single(t_quote_vars *vars,
									char *str, int *i);
...
void							handle_nested_single(t_struct_var *structs,
									t_quote_vars *vars, char *str, int *i);
void							status_pointer(int status,
									int *last_status_ptr);
void							exit_message(t_shell *elem);
void							do_exit(t_shell *element);
void							free_combined(int *exit_status, t_env *env);
void							do_heredoc(t_cmd *t_head, t_env *env,
									int *exit_status, int *fd_val);
void							execute_loop(t_shell *elem, t_env **env,
									int *fd_val, int **last_status_ptr_out);
...
void							init_token_flags(t_token *token, int new_word,
									int type, int quote);
...
void							prepare_pipe(int next_pipe[2], int need,
									int *next_read, int *next_write);
void							init_rw(t_shell *elem, int *need_next,
									int *next_read, int *next_write);
void							close_prev_next(int *prev_fd, int next_read,
									int next_write);
...
void							handle_single(t_quote_vars *vars, int *i,
									char *str);
void							cmd_loop(t_token **tokens, t_cmd **cmds,
									int type, t_env *envp);
void							free_cmds(t_cmd *head);
void							handle_sigint(int sig_num);
void							perr_exit(int errnum, char *cmd);
void							exec_command(t_shell *elem, t_env **env,
									char **envp);
void							delete_node(t_env **env, t_env *target,
									t_env *prev);
...
void							free_cmds(t_cmd *head);
void							do_commands(t_shell *elem, t_env **env,
									int fd_val);
...
void							handle_double(t_quote_vars *vars, int *i,
									char *str, t_struct_var *structs);
...
void							flush_to_head(t_struct_var *structs,
									t_quote_vars *vars);
void							assign_ctl_tokens(t_token **token, t_cmd **cmd,
									t_env *envp);
void							handle_quote(char *str, int *i, int type,
									t_struct_var *structs);
void							handle_empty_quotes(int *i, int new_word,
									t_token **head, t_token **current);
void							add_arr(t_quote_vars *vars,
									t_struct_var *structs);
void							handle_slash(t_token **head, t_token **current,
									char *str, int *i);
...
void							quote_if(char *str, t_token **head,
									t_token **current, int *i);
void							shift_left(char **arr);
void							shift_left_and_free(char **arr);
void							handle_join(t_cmd *cmds, int i);
void							type_if(int type, t_cmd **cmds,
									t_token **tokens, t_env *envp);
void							handle_slash(t_token **head, t_token **current,
									char *str, int *i);
void							handle_redirect(t_token **token, t_cmd **cmd,
									int type);
...
char							*ret_exp(int i, char *str, t_env *env,
									char *tmp);
...
pid_t							command_fork(t_shell *elem, t_env **env,
									int *prev_fd);
...
t_env							*create_env_node(char *arg, int flag,
									char *str);
t_cmd							*new_cmd_token(t_token *tokens);
t_cmd							*init_cmds(t_token *tokens, int exit_code,
									t_env *env);
